import { TokenizerConfig, DefaultTokenizerConfig } from './tokenizer_config'
...
export class Tokenizer {
    ...
    constructor(config: TokenizerConfig) {
        ...
        const STRING_TT = new TokenType('string', getStringRegexp(config.STRING_TYPES))
        const RESERVED_WORDS_TT = new TokenType('reserved_words', regexpFromWords(config.RESERVED_WORDS))
        const IDENTIFIER_TT = new TokenType('identifier', new RegExp(`^(${wordRegexp}(\\.${wordRegexp})+)`, 'i'))
        const OPERATOR_TT = new TokenType('operator', /^(\+|\-|\*|\/|%|=|!=|<>|>|>=|<|<=|::|\.)/)
        const WORD_TT = new TokenType('word', new RegExp(`^${wordRegexp}`, 'i'))
        this.tokenTypes = [
            ONE_LINE_COMMENT_TT,
            WHITESPACE_TT,
            COMMA_TT,
