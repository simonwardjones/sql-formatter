import { RESERVED_WORDS, ONE_LINE_COMMENT_SYMBOLS } from './restricted_words'
...
function regexpFromWords(words: string[]): RegExp {
    return RegExp('^(' + words.join('|') + ')')
}
function oneLineComments(starters: string[]) {
...
const wordRegexp = `[${wordUTF8}0-9]+`

export const TokenTypes: TokenType[] = [
    new TokenType('one_line_comment', oneLineComments(ONE_LINE_COMMENT_SYMBOLS)),
    // TODO multilincomment
    // TODO strings
    new TokenType('whitespace', /^(\s+)/),
    new TokenType('comma', /^,/),
    new TokenType('open_parenthesis', /^\(/),
    new TokenType('close_parenthesis', /^\)/),

    new TokenType('reserved_words', regexpFromWords(RESERVED_WORDS)),
    new TokenType('identifier', new RegExp(`^(${wordRegexp}(\\.${wordRegexp})+)`, 'i')),

    new TokenType('operator', /^(\+|\-|\*|\/|%|=|!=|<>|>|>=|<|<=|::)/),
    new TokenType('word', new RegExp(`^${wordRegexp}`, 'i')),
]
...
export class Tokeneizer {
    ...
    tokeneize(input: string): Token[] {
        let query: string = input
        while (query.length) {
            if (this.error) {
                return this.tokens
            }
            query = this.eatToken(query)
    ...
    eatToken(input: string): string {
        ...
        if (token === undefined) {
            console.log('WARNING - NOT ABLE TO FIND TOKEN')
            this.error = 1
            return remainingInput
        }
        return remainingInput
...
var demoTokenizer = new Tokeneizer(TokenTypes)
console.log('Tokens:')
...
const sqlExample = `
  select T.column1 as col, 
   ...
   'this is a -- string',
   $$this is tooooo$$,
   row_number over (partition by d order by 3 desc),
   (23-54.3) / 7 as dead
  sum(T.column2) as 
  ...
  and x >= 32
  `
console.log(demoTokenizer.tokeneize(sqlExample))